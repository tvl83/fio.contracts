...
#define TOKEN_CONTRACT "fiotokenwrap"_n

namespace fiowrapper {
    class [[eosio::contract("fio.tokenwrap")]] fiotokenwrap : public contract {
    private:
    ...
    public:
        ...
        fiotokenwrap(name s, name code, datastream<const char *> ds);

        /* Fungible token transfer (only trilium) */
        [[eosio::on_notify("fiotokenwrap::transfer")]] void transfer(name from, name to, asset quantity, string memo);
